...
io.on('connection', function(socket){
  	...
  	socket.on('audioObjectReceiver', function(){
		//finds EVERY audio object
		 ...
		 //for each object in res, populate the x/y/z/, put a sphere, decode the mp3
		 //new files shoudl be emitted to all users when they're made, as well as pushed to the initial check
  	...
  	socket.on('positionTest', function(dataBlob){
  		console.log(Buffer.isBuffer(dataBlob['audioMessage']));
  		...
  		console.log(audioMessage1);
			...
			audioMessage1.save(function (err, userObj) {
			  			if (err) {
			    			...
			    			console.log('saved successfully:', dataBlob['id']);
			  			}
					});
